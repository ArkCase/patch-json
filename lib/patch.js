...
/**
 * Check if was used Json Path notation
 * @param path
 * @returns {boolean}
 */
...
function actionSet(json, path, a) {
    if (isJsonPath(path)) {
        let childIndex = path.lastIndexOf('.');

        let parentPath = path.slice(0, childIndex);
        let childPath = path.slice(childIndex + 1);

        // Can't apply jsonpath.apply to the root
        if (parentPath === '$') {
            json[childPath] = a.VALUE;
        } else {
            jsonpath.apply(json, parentPath, (parent) => {
                parent[childPath] = a.VALUE;
                return parent;
            });
        }
    } else {
